 ...
 * (`datacenter`/`motion`/`tokens`/`branches` are bespoke, reached only via an explicit `thumb`).
 *
 ...
 */
export const MOTIF_KEYS = ['code', 'audio', 'writing', 'reading', 'process', 'feed', 'datacenter', 'motion', 'tokens', 'branches'] as const;
